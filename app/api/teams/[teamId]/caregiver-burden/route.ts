...
export async function POST(
  ...
  try {
    ...
    if (!membership.isAdmin || (membership.teamRole !== 'CAREGIVER' && membership.teamRole !== 'FAMILY')) {
      return NextResponse.json(
        { error: 'Unauthorized', message: 'Only caregivers and family members with admin access can complete this questionnaire.' },
        { status: 403 }
      )
    }
    ...
    const totalScore = Object.values(responses).reduce((sum, score) => sum + (score as number), 0)
