...
export default function ContactsPage() {
  ...
  }, [fetchContacts, activeTeam, fetchTeamData]);
  ...
  const canCreateContacts =
    ...
    (teamData?.currentUser.canCreateContacts ?? false);
  const canEditContacts =
    teamData?.currentUser.isAdmin ||
    (teamData?.currentUser.canEditContacts ?? false);
  ...
  const handleOpenForm = (contact?: Contact) => {
    if (contact) {
      ...
      if (!canEditContacts) {
        toast.error(
          "You do not have permission to edit contacts. Please contact your team admin.",
        );
        return;
      }
      ...
      if (!canCreateContacts) {
        toast.error(
          "You do not have permission to create contacts. Please contact your team admin.",
        );
        return;
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          toast.error(
            errorData.message || "You do not have permission to save contacts",
          );
        } else {
          toast.error(
            errorData.error || errorData.message || "Failed to save contact",
          );
        }
        throw new Error(
          errorData.error || errorData.message || "Failed to save contact",
  ...
  const getContactTypeInfo = (type: string) => {
    return (
      contactTypes.find((t) => t.value === type) ||
  ...
  return (
    <PermissionGuard permission="canViewContacts">
