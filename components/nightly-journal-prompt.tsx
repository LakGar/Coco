...
export function NightlyJournalPrompt() {
  ...
  const [lastEntry, setLastEntry] = React.useState<RoutineInstance | null>(
    null,
  );
  const [loading, setLoading] = React.useState(true);
  const [hasCheckedToday, setHasCheckedToday] = React.useState(false);
  const [completedRoutinesToday, setCompletedRoutinesToday] = React.useState<
    Set<string>
  >(new Set());

  // Reset completed routines at the start of each day
  React.useEffect(() => {
    const today = new Date().toISOString().split("T")[0];
    const lastCheckDate = localStorage.getItem("journalLastCheckDate");
    if (lastCheckDate !== today) {
      setCompletedRoutinesToday(new Set());
      localStorage.setItem("journalLastCheckDate", today);
    }
  }, []);
    ...
    const checkTimeAndRoutines = async () => {
      ...
      try {
        ...
        for (const routine of activeRoutines) {
          // Skip routines that were already completed today in this session
          if (completedRoutinesToday.has(routine.id)) {
            continue;
          }
          ...
          if (instancesResponse.ok) {
            ...
            // If we can't check, assume it needs an entry (unless already completed)
            routinesNeedingEntry.push(routine);
          }
        }
        ...
        if (routinesNeedingEntry.length > 0) {
          ...
          try {
            ...
            if (lastEntryResponse.ok) {
              ...
              if (
                lastEntryData.instances &&
                lastEntryData.instances.length > 0
              ) {
  ...
  }, [activeTeam, user, hasCheckedToday, completedRoutinesToday]);
  ...
  const handleSave = async (entry: RoutineInstance) => {
    ...
    try {
      ...
      // Mark this routine as completed today
      setCompletedRoutinesToday((prev) => new Set(prev).add(entry.routineId));
      ...
      // After a short delay, check if there are other routines that need entries
      // This allows the user to complete multiple routines if needed
      setTimeout(() => {
        setHasCheckedToday(false); // Allow checking for other routines
      }, 500);
    } catch (error) {
      if (process.env.NODE_ENV === "development") {
