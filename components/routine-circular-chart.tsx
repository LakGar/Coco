  ...
  const getWeekCompletion = () => {
    ...
    sortedDays.forEach((dayIndex) => {
      ...
      const hasEntry = instances.some((instance) => {
        const instanceDate = new Date(instance.entryDate)
          .toISOString()
          .split("T")[0];
      ...
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        {sortedDays.map((dayIndex, index) => {
          ...
          const startAngleOffset =
            (cornerRadius / innerRadius) * (180 / Math.PI);
          const endAngleOffset = (cornerRadius / innerRadius) * (180 / Math.PI);

          const startAngleRadRounded =
            ((startAngle + startAngleOffset) * Math.PI) / 180;
          const endAngleRadRounded =
            ((endAngle - endAngleOffset) * Math.PI) / 180;

          const startXInnerRounded =
            size / 2 + innerRadius * Math.cos(startAngleRadRounded);
          const startYInnerRounded =
            size / 2 + innerRadius * Math.sin(startAngleRadRounded);
          const endXInnerRounded =
            size / 2 + innerRadius * Math.cos(endAngleRadRounded);
          const endYInnerRounded =
            size / 2 + innerRadius * Math.sin(endAngleRadRounded);

          const startXOuterRounded =
            size / 2 + outerRadius * Math.cos(startAngleRadRounded);
          const startYOuterRounded =
            size / 2 + outerRadius * Math.sin(startAngleRadRounded);
          const endXOuterRounded =
            size / 2 + outerRadius * Math.cos(endAngleRadRounded);
          const endYOuterRounded =
            size / 2 + outerRadius * Math.sin(endAngleRadRounded);
