...
>(({ defaultOpen = true, open: openProp, onOpenChange, className, style, children, ...props }, ref) => {
  ...
  const [sidebarState, setSidebarState] = React.useState<"expanded" | "collapsed">(
    defaultOpen ? "expanded" : "collapsed"
  )

  // This is a simple implementation for mobile detection
  ...
  const open = openProp ?? sidebarState === "expanded"

  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      const newOpen = typeof value === "function" ? value(open) : value
      setSidebarState(newOpen ? "expanded" : "collapsed")
  ...
  const contextValue = React.useMemo<SidebarContext>(
    ...
    [sidebarState, open, setOpen, openMobile, setOpenMobile, isMobile, toggleSidebar]
  )
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "duration-200 relative h-svh w-[--sidebar-width] bg-sidebar border-r border-sidebar-border transition-[width] ease-linear",
      ...
      >
        <div
          data-sidebar="sidebar"
          className="flex h-full w-full flex-col group-data-[collapsible=icon]:items-center"
        >
          {children}
        </div>
      </div>
...
>(({ className, ...props }, ref) => {
  return (
    <input
      ...
      data-sidebar="input"
      className={cn(
        "flex h-8 w-full rounded-md border border-sidebar-border bg-sidebar px-3 py-1 text-sm text-sidebar-foreground ring-offset-sidebar transition-[width] placeholder:text-sidebar-foreground/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar disabled:cursor-not-allowed disabled:opacity-50 group-data-[collapsible=icon]:!w-8 group-data-[collapsible=icon]:!px-2",
        className
      )}
      {...props}
    />
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        showOnHover &&
          "group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 peer-data-[state=open]/menu-button:opacity-100 data-[state=open]:opacity-100 group-data-[collapsible=icon]:opacity-100 opacity-0 transition-opacity",
        className
      )}
      {...props}
    />
  ...
  return (
    ...
    >
      {showIcon && (
        <div
          className={cn(
            "rounded-md h-4 w-4 bg-sidebar-primary/10",
            "group-data-[collapsible=icon]:hidden"
          )}
        />
      )}
      <div
        className={cn(
          "rounded-md h-4 flex-1 max-w-[--skeleton-width] bg-sidebar-primary/10",
          "group-data-[collapsible=icon]:hidden"
        )}
        style={
          {
...
>(({ asChild = false, isActive = false, size = "default", className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        sidebarMenuButtonVariants({ size, variant: "ghost" }),
        "h-7 w-full pl-2 text-sidebar-foreground/70",
        className
      )}
      {...props}
    />
