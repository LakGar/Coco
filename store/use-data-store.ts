    ...
    (set, get) => ({
      ...
      fetchTeamData: async (teamId, force = false) => {
        ...
        const hasValidCache =
          cached !== null &&
          cached !== undefined &&
          now - timestamp < state.teamDataCacheTTL;
      ...
      fetchTasks: async (teamId, force = false) => {
        ...
        const hasValidCache =
          ...
          now - timestamp < state.tasksCacheTTL;
      ...
      fetchRoutines: async (teamId, force = false) => {
        ...
        const hasValidCache =
          cached !== undefined &&
          Array.isArray(cached) &&
          now - timestamp < state.routinesCacheTTL;
      ...
      fetchNotes: async (teamId, force = false) => {
        ...
        const hasValidCache =
          cached !== undefined &&
          Array.isArray(cached) &&
          now - timestamp < state.notesCacheTTL;
      ...
      fetchMoods: async (teamId, force = false) => {
        ...
        const hasValidCache =
          cached !== undefined &&
          Array.isArray(cached) &&
          now - timestamp < state.moodsCacheTTL;
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : "Failed to fetch moods";
          set({
            loading: { ...state.loading, [key]: false },
            errors: { ...state.errors, [key]: errorMessage },
          });
      ...
      addRoutine: (teamId, routine) => {
        ...
        set({
          routines: {
            ...state.routines,
            [teamId]: [routine, ...currentRoutines],
          },
        });
      },
